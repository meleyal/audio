...
# ---------------------------------------------------------------------------
# Pure-PyTorch neighborhood attention fallback (replaces NATTEN dependency).
# Implements the same split QK-with-RPB + AV interface that allin1 expects.
# ---------------------------------------------------------------------------

def _gather_neighbors_1d(x, T, K, half_k, dilation):
    """Gather K dilated neighbors for each of the T positions. Returns (*, T, K, D)."""
    offsets = (torch.arange(K, device=x.device) - half_k) * dilation  # (K,)
    t_idx = torch.arange(T, device=x.device)                           # (T,)
    gather_idx = (t_idx.unsqueeze(1) + offsets.unsqueeze(0)).clamp(0, T - 1)  # (T, K)
    return x[..., gather_idx, :]  # (..., T, K, D)


def _na1d_qk_rpb(query, key, rpb, kernel_size, dilation):
    """1-D neighborhood attention QK + RPB.
    query/key: (B, H, T, D)  rpb: (H, 2K-1)  →  (B, H, T, K)
    """
    B, H, T, D = query.shape
    K, half_k = kernel_size, kernel_size // 2

    key_flat = key.reshape(B * H, T, D)
    key_neighbors = _gather_neighbors_1d(key_flat, T, K, half_k, dilation)  # BH, T, K, D
    key_neighbors = key_neighbors.reshape(B, H, T, K, D)

    scores = (query.unsqueeze(3) * key_neighbors).sum(-1)  # B, H, T, K

    rpb_idx = torch.arange(K, device=rpb.device) + (K - 1 - half_k)
    scores = scores + rpb[:, rpb_idx].unsqueeze(0).unsqueeze(2)
    return scores


def _na1d_av(attn_weights, value, kernel_size, dilation):
    """1-D neighborhood attention AV.
    attn_weights: (B, H, T, K)  value: (B, H, T, D)  →  (B, H, T, D)
    """
    B, H, T, K = attn_weights.shape
    D, half_k = value.shape[-1], K // 2

    val_flat = value.reshape(B * H, T, D)
    val_neighbors = _gather_neighbors_1d(val_flat, T, K, half_k, dilation)  # BH, T, K, D
    val_neighbors = val_neighbors.reshape(B, H, T, K, D)

    return (attn_weights.unsqueeze(-1) * val_neighbors).sum(-2)  # B, H, T, D


def _gather_neighbors_2d(x, Ht, Wt, K, half_k, dilation):
    """Gather K×K dilated neighbors for each of the Ht×Wt positions.
    x: (BH, Ht, Wt, D)  →  (BH, Ht, Wt, K*K, D)
    """
    offsets = (torch.arange(K, device=x.device) - half_k) * dilation  # (K,)
    h_idx = torch.arange(Ht, device=x.device)
    w_idx = torch.arange(Wt, device=x.device)
    r_idx = (h_idx.unsqueeze(1) + offsets.unsqueeze(0)).clamp(0, Ht - 1)  # Ht, K
    c_idx = (w_idx.unsqueeze(1) + offsets.unsqueeze(0)).clamp(0, Wt - 1)  # Wt, K
    # Build explicit (Ht, K, Wt, K) index arrays so broadcasting is unambiguous
    row_idx = r_idx[:, :, None, None].expand(Ht, K, Wt, K)   # Ht, K, Wt, K
    col_idx = c_idx[None, None, :, :].expand(Ht, K, Wt, K)   # Ht, K, Wt, K
    # x[:, (Ht,K,Wt,K), (Ht,K,Wt,K), :] → BH, Ht, K, Wt, K, D
    neighbors = x[:, row_idx, col_idx, :]
    # BH, Ht, K, Wt, K, D → BH, Ht, Wt, K, K, D → BH, Ht, Wt, K*K, D
    return neighbors.permute(0, 1, 3, 2, 4, 5).reshape(x.shape[0], Ht, Wt, K * K, x.shape[-1])


def _na2d_qk_rpb(query, key, rpb, kernel_size, dilation):
    """2-D neighborhood attention QK + RPB.
    query/key: (B, H, Ht, Wt, D)  rpb: (H, 2K-1, 2K-1)  →  (B, H, Ht, Wt, K*K)
    """
    B, H, Ht, Wt, D = query.shape
    K, half_k = kernel_size, kernel_size // 2

    key_flat = key.reshape(B * H, Ht, Wt, D)
    key_neighbors = _gather_neighbors_2d(key_flat, Ht, Wt, K, half_k, dilation)  # BH, Ht, Wt, K*K, D
    key_neighbors = key_neighbors.reshape(B, H, Ht, Wt, K * K, D)

    scores = (query.unsqueeze(4) * key_neighbors).sum(-1)  # B, H, Ht, Wt, K*K

    rpb_idx = torch.arange(K, device=rpb.device) + (K - 1 - half_k)
    bias2d = rpb[:, rpb_idx.unsqueeze(1), rpb_idx.unsqueeze(0)]  # H, K, K
    scores = scores + bias2d.reshape(1, H, 1, 1, K * K)
    return scores


def _na2d_av(attn_weights, value, kernel_size, dilation):
    """2-D neighborhood attention AV.
    attn_weights: (B, H, Ht, Wt, K*K)  value: (B, H, Ht, Wt, D)  →  (B, H, Ht, Wt, D)
    """
    B, H, Ht, Wt, KK = attn_weights.shape
    K, half_k = int(KK ** 0.5), int(KK ** 0.5) // 2
    D = value.shape[-1]

    val_flat = value.reshape(B * H, Ht, Wt, D)
    val_neighbors = _gather_neighbors_2d(val_flat, Ht, Wt, K, half_k, dilation)  # BH, Ht, Wt, K*K, D
    val_neighbors = val_neighbors.reshape(B, H, Ht, Wt, K * K, D)

    return (attn_weights.unsqueeze(-1) * val_neighbors).sum(-2)  # B, H, Ht, Wt, D


natten1dqkrpb = _na1d_qk_rpb
natten1dav    = _na1d_av
natten2dqkrpb = _na2d_qk_rpb
natten2dav    = _na2d_av
